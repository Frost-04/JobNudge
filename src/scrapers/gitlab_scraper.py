...
class GitLabScraper(BaseScraper):
    """
    Scraper for GitLab job board (powered by Greenhouse).
    ...
    relevant roles (Engineering department with id 4011044002).
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich GitLab job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "GitLab"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        ...
        https://job-boards.greenhouse.io/gitlab/jobs/8537864002
    ...
    def _extract_job_id_from_url(self, url: str) -> str:
        """
        Greenhouse URLs: https://job-boards.greenhouse.io/gitlab/jobs/8537864002
        """
        if not url:
            return ""
    ...
    async def _fallback_links(self, page: Page, source_url: str, max_jobs: int) -> list[Job]:
        ...
        for link in soup.select('a[href*="/gitlab/jobs/"]'):
            if len(jobs) >= max_jobs:
                break

            href = link.get("href")

            if not href or "/gitlab/jobs/" not in str(href):
                continue
    ...
    def _clean_multiline_text(text: str) -> str:
        ...
        for line in text.splitlines():
            clean_line = GitLabScraper._clean_text(line)
