...
class SamsungScraper(BaseScraper):
    """
    Scraper for Samsung Careers job search pages (Workday platform).
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if not cards:
                self.logger.warning("No Samsung job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Samsung job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list (R-number like "R117344").
        job_id = ""
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Samsung"),
            title=title,
            location=location,
            url=url,
