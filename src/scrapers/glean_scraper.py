...
class GleanScraper(BaseScraper):
    """
    Scraper for Glean job board (powered by Greenhouse).
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Glean job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Glean"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        ...
        https://job-boards.greenhouse.io/gleanwork/jobs/4612849005
    ...
    def _extract_job_id_from_url(self, url: str) -> str:
        """
        Greenhouse URLs: https://job-boards.greenhouse.io/gleanwork/jobs/4612849005
        """
        if not url:
            return ""
    ...
    async def _fallback_links(self, page: Page, source_url: str, max_jobs: int) -> list[Job]:
        ...
        for link in soup.select('a[href*="/gleanwork/jobs/"]'):
            if len(jobs) >= max_jobs:
                break

            href = link.get("href")

            if not href or "/gleanwork/jobs/" not in str(href):
                continue
    ...
    def _clean_multiline_text(text: str) -> str:
        ...
        for line in text.splitlines():
            clean_line = GleanScraper._clean_text(line)
