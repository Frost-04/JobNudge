...
class AkamaiScraper(BaseScraper):
    """
    Scraper for Akamai Oracle Cloud Candidate Experience pages.

    Akamai uses the Oracle HCM Cloud Candidate Experience platform
    (same as JPMorgan Chase, Chubb, American Express).

    Expected listing card structure:

        ul.jobs-grid__list
          li[data-qa="searchResultItem"]
            div.job-tile.job-grid-item
    ...
    Detail pages open as a popup/overlay on the listing page, but direct
    URL access works — no captcha/interstitial blocking.
    """
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if card_count == 0:
                self.logger.warning("No Akamai job cards found after render")
            ...
            for card in cards[:max_jobs]:
                ...
                # Enrich with detail page data (description).
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Akamai job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Akamai"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        """Extract job ID from URL path /job/{id}/ or aria-labelledby attribute."""
        if link:
    ...
    def _extract_location(self, card: Tag) -> str:
        ...
        if posting_locations:
            ...
            if primary_span:
                ...
                if location_text:
                    # Remove workplace type suffix like "(Remote)"
                    location_text = re.sub(r'\s*\(.*?\)\s*', '', location_text).strip()
                    return location_text

        # Fallback: look for location label/value pair in job-info items.
        job_info_items = card.select("li.job-list-item__job-info-item")
        for item in job_info_items:
            label_el = item.select_one(
                "div.job-list-item__job-info-label--locations, "
                "div.job-list-item__job-info-label"
            )
            if label_el and "location" in label_el.get_text(" ").lower():
                value_el = item.select_one("div.job-list-item__job-info-value")
                if value_el:
                    text = self._clean_location_text(value_el.get_text())
                    if text:
                        return text
            # Also check if this is the first item (usually location).
            if job_info_items.index(item) == 0:
                value_el = item.select_one("div.job-list-item__job-info-value")
                if value_el:
                    text = self._clean_location_text(value_el.get_text())
                    if text:
    ...
    def _extract_posted_date_from_card(self, card: Tag) -> str:
        ...
        The card shows posting date like '12/06/2026' in a
        div.job-list-item__job-info-value that follows a label
        with class job-list-item__job-info-label--posting-date.
        """
        job_info_items = card.select("li.job-list-item__job-info-item")
        for item in job_info_items:
            label_el = item.select_one("div.job-list-item__job-info-label--posting-date")
            if label_el:
                value_el = item.select_one("div.job-list-item__job-info-value")
                if value_el:
                    date_text = self._clean_text(value_el.get_text())
                    # Expect format like "12/06/2026"
                    match = re.search(r'\d{1,2}/\d{1,2}/\d{4}', date_text)
                    if match:
                        return match.group(0)

        # Fallback: regex search over full card text.
    ...
    def _extract_detail_description(self, soup: Tag) -> str:
        ...
        for tag in desc_el.select("table, script, style, img, svg"):
            tag.decompose()

        # Remove "Connect with us" social-media rows.
        for tr in desc_el.select("tr"):
            text = self._clean_text(tr.get_text())
            if any(
    ...
    def _clean_location_text(text: str) -> str:
        if not text:
            return ""
        text = text.replace("\xa0", " ")
        # Remove workplace type suffixes like "(Remote)", "(On-site)".
    ...
    def _extract_job_id_from_url(self, url: str) -> str:
        """Extract the numeric job ID from a URL path like /job/3143/."""
