...
class CadenceScraper(BaseScraper):
    """Scraper for Cadence careers (Workday platform, wd1).
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if not cards:
                self.logger.warning("No Cadence job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Cadence job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list items (e.g. "R52496").
        job_id = ""
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Cadence"),
            title=title,
            location=location,
            url=url,
