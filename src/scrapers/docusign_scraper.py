...
class DocusignScraper(BaseScraper):
    """
    Scraper for DocuSign Careers search result pages.
      ...
      mat-expansion-panel-header
        ...
        mat-panel-description
          .description-container
            .job-card-result-container
              span.label-value.location        (first one has text, second is empty)
        ...
        try:
            ...
            for card in cards[:max_jobs]:
                ...
                try:
                    ...
                    self.logger.warning(
                        "Failed to enrich DocuSign job detail page %s: %s",
                        job.url,
                        exc,
                    )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "DocuSign"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_link(self, card: Tag, source_url: str) -> str:
        ...
        return self._make_docusign_job_url(source_url, str(href))
    ...
    def _extract_location(self, card: Tag) -> str:
        """
        DocuSign cards have TWO span.label-value.location elements:
        - First one: contains the actual location text ("Bengaluru, Karnataka\\nIndia")
        - Second one: empty (itemprop="jobLocation" with no text)

        We take the first non-empty location span text, first line only.
        """
        ...
        for el in els:
            ...
            # Take the first meaningful line (city, state)
            lines = [
                line.strip()
    ...
    def _extract_job_id_from_card(self, card: Tag, link: str) -> str:
        """
        DocuSign job URLs look like: /careers-home/jobs/29301?lang=en-us

        No separate req-id span exists on DocuSign cards, so only URL-based extraction.
        """

        if link:
            job_id = self._extract_docusign_job_id_from_url(link)
    ...
    def _format_detail_metadata(self, detail_data: dict[str, str]) -> str:
        """DocuSign detail pages don't have separate metadata fields.
    ...
    def _make_docusign_job_url(self, source_url: str, href: str) -> str:
    ...
    def _extract_docusign_job_id_from_url(self, url: str) -> str:
        if not url:
            return ""
    ...
    async def _fallback_links(self, page: Page, source_url: str, max_jobs: int) -> list[Job]:
        ...
        for link in soup.select(self.TITLE_LINK_SELECTOR):
            ...
            url = self._make_docusign_job_url(source_url, str(href))
            ...
            job_id = self._extract_docusign_job_id_from_url(url)
