...
class RedhatScraper(BaseScraper):
    """
    Scraper for Red Hat Careers job search pages (Workday platform).
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if not cards:
                self.logger.warning("No Red Hat job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Red Hat job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list (R-number like "R-057024").
        job_id = ""
        subtitle_items = card.select(self.JOB_ID_SELECTOR)
        for item in subtitle_items:
            text = self._clean_text(item.get_text())
            if text.upper().startswith("R-") or text.upper().startswith("JR"):
                job_id = text
                break
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Red Hat"),
            title=title,
            location=location,
            url=url,
