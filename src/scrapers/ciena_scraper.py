...
class CienaScraper(BaseScraper):
    """
    Scraper for Ciena Careers job search pages (Workday platform).
    ...
    _R_NUMBER_RE = re.compile(r"^(R\-?\d+)", re.IGNORECASE)
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if not cards:
                self.logger.warning("No Ciena job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Ciena job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Ciena"),
            title=title,
            location=location,
            url=url,
