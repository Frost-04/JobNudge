...
class ChubbScraper(BaseScraper):
    """
    Scraper for Chubb Oracle Cloud Candidate Experience pages.
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if card_count == 0:
                self.logger.warning("No Chubb job cards found after render")
            ...
            for card in cards[:max_jobs]:
                ...
                # Enrich with detail page data (description + posted date).
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Chubb job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Chubb"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_location(self, card: Tag) -> str:
        ...
        if posting_locations:
            locations = self._extract_locations_from_posting_locations(posting_locations)
            if locations:
    ...
    def _extract_locations_from_posting_locations(self, node: Tag) -> list[str]:
        ...
        # Secondary locations from aria-label.
    ...
    def _extract_posted_date_from_card(self, card: Tag) -> str:
        """Cards may show posted date as 'Posted on DD/MM/YYYY'."""
