...
class AtlanScraper(BaseScraper):
    """
    Scraper for Atlan jobs (powered by AshbyHQ ATS).

    Uses the AshbyHQ native board at ``jobs.ashbyhq.com/atlan`` with
    pre-filtered URL params (Engineering department + India location),
    so no client-side location filtering is needed.
    ...
    CARD_SELECTOR = 'div.ashby-job-posting-brief-list a[href*="/atlan/"]'

    JOB_CARD_SELECTORS = [
        'div.ashby-job-posting-brief-list a[href*="/atlan/"]',
        'a[href*="/atlan/"][href$="/"]',
            ...
            for card in cards:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Atlan job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Atlan"),
            title=title,
            location=location,
            url=link,
    ...
    async def _fallback_links(self, page: Page, source_url: str, max_jobs: int) -> list[Job]:
        ...
        for link in soup.select('a[href*="/atlan/"]'):
            if len(jobs) >= max_jobs:
                break
            href = link.get("href")
            if not href or "/atlan/" not in str(href):
                continue
