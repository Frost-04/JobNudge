...
class SandiskScraper(BaseScraper):
    """
    Scraper for Sandisk careers via SmartRecruiters platform.
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
                ...
                for card in job_items[:max_jobs]:
                    ...
                    if self._should_exclude(job.title):
                        ...
                        try:
                            ...
                            self.logger.warning(
                                "Failed to enrich Sandisk detail page %s: %s",
                                job.url,
                                exc,
                            )
            ...
            self.logger.info("Total Sandisk jobs scraped: %d", len(jobs))
            return jobs
    ...
    def _parse_card(self, card: Tag, source_url: str, location_name: str = "") -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Sandisk"),
            title=title,
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        ...
        https://jobs.smartrecruiters.com/Sandisk/744000112177039-staff-engineer-...
        The numeric portion after /Sandisk/ is the job ID.
        """
        if link:
            match = re.search(r"/Sandisk/(\d+)", link)
            if match:
