...
class EricssonScraper(BaseScraper):
    """
    Scraper for Ericsson Careers search result pages (Phenom People platform).
    ...
    div[data-test-id="job-listing"]
      a[href^="/careers/job/"]
        div.title-1aNJK
        div.fieldValue-3kEar  (location — in first fieldsContainer with SVG icon)
        div.fieldValue-3kEar  (department — in second fieldsContainer, no icon)
        div.subData-13Lm1     (posted date)

    Expected detail structure:

    div.detailContainer-2qNET
      div.detailLabel-2AsIg  -> "Job ID"
      div.detailValue-3NGwm  -> "772960"
    ...
    FIELD_CONTAINER_SELECTOR = 'div[class^="fieldsContainer-"], div[class*=" fieldsContainer-"]'
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Ericsson job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Ericsson"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_link(self, card: Tag, source_url: str) -> str:
        ...
        return self._make_ericsson_job_url(source_url, str(href))
    ...
    def _extract_location(self, card: Tag) -> str:
        """
        Ericsson cards have two fieldValue elements:
        1. Location (inside fieldsContainer with an SVG icon)
        2. Department (second fieldsContainer, no icon)

        Only return the location by finding the first fieldsContainer
        that contains an SVG icon.
        """
        field_containers = card.select(self.FIELD_CONTAINER_SELECTOR)

        for container in field_containers:
            # Location fieldsContainer has an SVG icon.
            if container.select_one("svg"):
                value_el = container.select_one(self.FIELD_VALUE_SELECTOR)

                if value_el:
                    return self._clean_location_text(value_el.get_text())

        # Fallback: take the very first fieldValue in the card.
        all_values = card.select(self.FIELD_VALUE_SELECTOR)

        if all_values:
            return self._clean_location_text(all_values[0].get_text())
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        """Ericsson listing href looks like: /careers/job/563121772208395"""

        if link:
            job_id = self._extract_ericsson_job_id_from_url(link)
    ...
    def _make_ericsson_job_url(self, source_url: str, href: str) -> str:
    ...
    def _extract_ericsson_job_id_from_url(self, url: str) -> str:
        if not url:
            return ""
    ...
    async def _fallback_links(
        ...
        for anchor in anchors[:max_jobs]:
            ...
            job_url = self._make_ericsson_job_url(source_url, str(href))
            job_id = self._extract_ericsson_job_id_from_url(job_url)
