...
class ThoughtspotScraper(BaseScraper):
    """
    Scraper for ThoughtSpot Careers job search pages (Workday platform).
    ...
    RESULTS_CONTAINER = 'ul[aria-label^="Page"][role="list"]'
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if not cards:
                self.logger.warning("No ThoughtSpot job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich ThoughtSpot job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list (JR number like "JR2176").
        job_id = ""
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "ThoughtSpot"),
            title=title,
            location=location,
            url=url,
