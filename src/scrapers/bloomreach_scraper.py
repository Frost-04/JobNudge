...
class BloomreachScraper(BaseScraper):
    """
    Scraper for Bloomreach job board (powered by Greenhouse, standard layout).
    ...
    Anthropic, Glean, InMobi, and Quince.

    The listing URL uses query-string department/office filters to show only
    relevant roles (Engineering) in India.
      ...
      div.job-posts--table--department
        ...
        div.job-posts--table
          table > tbody
            tr.job-post
              td.cell > a[href]
                p.body--medium             (job title)
                p.body--metadata           (location)
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Bloomreach job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Bloomreach"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        ...
        https://job-boards.greenhouse.io/bloomreach/jobs/6052712
    ...
    def _extract_job_id_from_url(self, url: str) -> str:
        """
        Greenhouse URLs: https://job-boards.greenhouse.io/bloomreach/jobs/6052712
        """
        if not url:
            return ""
    ...
    async def _fallback_links(self, page: Page, source_url: str, max_jobs: int) -> list[Job]:
        ...
        for link in soup.select('a[href*="/bloomreach/jobs/"]'):
            if len(jobs) >= max_jobs:
                break

            href = link.get("href")

            if not href or "/bloomreach/jobs/" not in str(href):
                continue
