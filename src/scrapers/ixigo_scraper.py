...
class IxigoScraper(BaseScraper):
    """
    Scraper for Ixigo careers via SmartRecruiters platform.

    The search page groups jobs by location in sections:

        section.openings-section.opening--grouped
          header > h3.opening-title          (e.g. "Gurugram, India")
          ul.opening-jobs
            li.opening-job.job
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
                ...
                for card in job_items[:max_jobs]:
                    ...
                    if self._should_exclude(job.title):
                        ...
                        try:
                            ...
                            self.logger.warning(
                                "Failed to enrich Ixigo detail page %s: %s",
                                job.url,
                                exc,
                            )
            ...
            self.logger.info("Total Ixigo jobs scraped: %d", len(jobs))
            return jobs
    ...
    def _parse_card(self, card: Tag, source_url: str, location_name: str = "") -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Ixigo"),
            title=title,
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        ...
        https://jobs.smartrecruiters.com/Ixigo/744000112177039-software-engineer-...
        The numeric portion after /Ixigo/ is the job ID.
        """
        if link:
            match = re.search(r"/Ixigo/(\d+)", link)
            if match:
