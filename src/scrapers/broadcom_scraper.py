...
class BroadcomScraper(BaseScraper):
    """Scraper for Broadcom careers (Workday platform, wd1).
    ...
    Job cards:
        ...
        li.css-1q2dra3
          ...
          [data-automation-id="subtitle"] li   (RXXXXXX)
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if not cards:
                self.logger.warning("No Broadcom job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Broadcom job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list items (e.g. "R026183" or "JR-0107786").
        job_id = ""
        subtitle_items = card.select(self.JOB_ID_SELECTOR)
        for item in subtitle_items:
            text = self._clean_text(item.get_text())
            # Workday R- or JR- prefixed requisition IDs
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Broadcom"),
            title=title,
            location=location,
            url=url,
