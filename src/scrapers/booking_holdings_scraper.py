...
class BookingHoldingsScraper(BaseScraper):
    """
    Scraper for Booking Holdings COE Careers search result pages.

    Expected listing card structure (Angular Material expansion panels —
    same iCIMS platform as AMD and DocuSign):

    mat-expansion-panel.search-result-item
      mat-expansion-panel-header
        mat-panel-title
          p.job-title
            a.job-title-link[href^="/bookingholdings-coe/jobs/"]
              span[itemprop="title"]
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Booking Holdings job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Booking Holdings"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_location(self, card: Tag) -> str:
        """Extract location from the first non-empty label-value.location span."""
        ...
        for el in els:
            ...
            # Take the first meaningful line (city, state/country).
            lines = [
                line.strip()
    ...
    def _extract_job_id_from_card(self, card: Tag, link: str) -> str:
        """
        Booking Holdings job URLs:
        /bookingholdings-coe/jobs/45207?lang=en-us
        """
        if link:
    ...
    def _make_job_url(self, source_url: str, href: str) -> str:
        ...
        if href.startswith("/bookingholdings-coe/jobs/"):
    ...
    def _extract_job_id_from_url(self, url: str) -> str:
        ...
        match = re.search(r"/bookingholdings-coe/jobs/(\d+)", url, flags=re.IGNORECASE)
    ...
    async def _fallback_links(self, page: Page, source_url: str, max_jobs: int) -> list[Job]:
        ...
        for link in soup.select(self.TITLE_LINK_SELECTOR):
            ...
            if not href or "/bookingholdings-coe/jobs/" not in str(href):
                continue
