...
class NikeScraper(BaseScraper):
    """
    Scraper for Nike job board (careers.nike.com).

    Custom job board with server-rendered listing cards and separate
    detail pages for each job.

    Expected listing structure:

    ul.results-list.front
      li.results-list__item
        div.results-list__item-header
          h3.results-list__item-title
            a.results-list__item-title--link[href]   (title + detail URL)
            span.reference                            (job ID, e.g. "R-85705")
        div.results-list__item-content
          div.results-list__item-street--label       (location)
          span.results-list__categories--label        (category)

    Expected detail page structure:

    section.job-description
      div.job-description-header      (company boilerplate — excluded)
      div.description                  (actual job description)
      div.job-description-footer      (footer boilerplate — excluded)

    Job IDs come from span.reference in the card (R-XXXXX format).
    Detail pages are at relative URLs like /title-slug/job/R-XXXXX.
    """

    CARD_SELECTOR = 'li.results-list__item'
    ...
    TITLE_SELECTOR = 'h3.results-list__item-title a.results-list__item-title--link'
    JOB_ID_SELECTOR = 'span.reference'
    LOCATION_SELECTOR = 'span.results-list__item-street--label'
    CATEGORY_SELECTOR = 'span.results-list__categories--label'

    DESCRIPTION_SELECTOR = 'section.job-description div.description'
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            # Scroll to bottom and back up to trigger any lazy content.
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Nike job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Nike"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_job_id(self, card: Tag) -> str:
        """Extract job ID from span.reference (e.g. "R-85705")."""
        el = card.select_one(self.JOB_ID_SELECTOR)

        if el:
            ref = self._clean_text(el.get_text())
            if ref:
                return ref

        # Fallback: extract R-XXXXX from the URL.
        link_el = card.select_one(self.TITLE_SELECTOR)
        if link_el:
            href = str(link_el.get("href", ""))
            match = re.search(r'/job/(R-\d+)', href, re.IGNORECASE)
            if match:
    ...
    async def _fallback_links(self, page: Page, source_url: str, max_jobs: int) -> list[Job]:
        ...
        for link in soup.select('a[href*="/job/R-"]'):
            if len(jobs) >= max_jobs:
                break

            href = link.get("href")

            if not href or "/job/R-" not in str(href):
                continue
            ...
            job_id_match = re.search(r'/job/(R-\d+)', href_str, re.IGNORECASE)
            ...
            if not self._should_exclude(title):
                try:
                    ...
                    self.logger.warning(
                        "Failed to enrich Nike fallback detail page %s: %s",
                        job_url,
                        exc,
                    )
