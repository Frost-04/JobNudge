...
class EmergentScraper(BaseScraper):
    """
    Scraper for Emergent job board (powered by Greenhouse).

    Standard Greenhouse board at job-boards.greenhouse.io/emergentlabsinc.
    ...
    (e.g. /emergentlabsinc/jobs/4086932009 → 4086932009).
    """
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Emergent job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Emergent"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        ...
        Standard Greenhouse URLs: /emergentlabsinc/jobs/4086932009
        """
        if link:
    ...
    async def _fallback_links(self, page: Page, source_url: str, max_jobs: int) -> list[Job]:
        ...
        for link in soup.select('td.cell > a[href*="/jobs/"]'):
            ...
            # Extract title from the first <p> child.
            title = ""
            title_el = link.select_one(self.TITLE_SELECTOR)
            if title_el:
            ...
            # Extract location from fallback anchor.
            location = ""
            ...
            if not self._should_exclude(title):
                try:
                    ...
                    self.logger.warning(
                        "Failed to enrich Emergent fallback detail page %s: %s",
                        job_url,
                        exc,
                    )
