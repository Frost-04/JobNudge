...
async def run_ai_pipeline() -> int:
    ...
    scraper_result = await run_main_scraper()

    if scraper_result != 0:
        logging.getLogger("job_alert_bot").error(
            "Scraper pipeline failed with code %d. Aborting AI pipeline.", scraper_result
        )
...
def main() -> None:
    sys.exit(asyncio.run(run_ai_pipeline()))
