...
class ClouderaScraper(BaseScraper):
    """
    Scraper for Cloudera Careers job search pages (Workday platform).
        ...
        section[data-automation-id="jobResults"]
          ul[aria-label="Page ..."][role="list"]
            li
              ...
              [data-automation-id="subtitle"] li   (numeric job ID)
        ...
        try:
            ...
            try:
                ...
                if await accept_btn.is_visible(timeout=5000):
                    ...
                    self.logger.debug("Accepted cookies on Cloudera page.")
                    await page.wait_for_timeout(2000)
            except Exception:
                self.logger.debug("No cookie consent popup on Cloudera page.")
            ...
            if not cards:
                self.logger.warning("No Cloudera job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Cloudera job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list (e.g. "260696").
        job_id = ""
        ...
        if not job_id:
            # Fallback: first numeric-only li in subtitle.
            for item in subtitle_items:
                text = self._clean_text(item.get_text())
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Cloudera"),
            title=title,
            location=location,
            url=url,
