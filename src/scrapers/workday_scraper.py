...
class WorkdayScraper(BaseScraper):
    """Scraper for Workday careers (Workday platform, wd5).
    ...
    Job cards:
        ...
        li.css-1q2dra3
          ...
          [data-automation-id="remoteType"] dd
          [data-automation-id="locations"] dd
          [data-automation-id="postedOn"] dd
          [data-automation-id="subtitle"] li   (JR-XXXXXXX)
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if not cards:
                self.logger.warning("No Workday job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Workday job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list items (e.g. "JR-0107786").
        job_id = ""
        subtitle_items = card.select(self.JOB_ID_SELECTOR)
        for item in subtitle_items:
            text = self._clean_text(item.get_text())
            # Workday JR- requisition IDs (with or without dash)
            if text.upper().startswith("JR") and any(ch.isdigit() for ch in text):
                job_id = text
                break
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Workday"),
            title=title,
            location=location,
            url=url,
