...
class HoneywellScraper(BaseScraper):
    """
    Scraper for Honeywell Oracle Cloud Candidate Experience pages.
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            # Use "commit" (fastest) — "domcontentloaded" can hang on Oracle Cloud pages
            # due to slow analytics/chat resources.
            await page.goto(source_url, wait_until="commit", timeout=30000)

            # Give Knockout.js time to render the full card DOM
            await page.wait_for_timeout(8000)

            # Verify cards are present via JS
            card_count = await page.evaluate(
                "() => document.querySelectorAll('span.job-tile__title').length"
            )
            if card_count == 0:
                self.logger.warning("No Honeywell job cards found after render")
            ...
            for card in cards[:max_jobs]:
                ...
                # Honeywell blocks direct detail URL access (redirects to captcha page).
                # Cards already contain short descriptions via p.job-list-item__description,
                # so we skip detail enrichment and keep card-level data.
                if self._should_exclude(job.title):
                    self.logger.debug("Excluding (card-only): %s", job.title)
    ...
    async def _wait_for_any_selector(
        self, page: Page, selectors: list[str], timeout_ms: int | None = None
    ) -> str | None:
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Honeywell"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_link(self, card: Tag, source_url: str) -> str:
        ...
        return self._make_honeywell_job_url(source_url, str(href))
    ...
    async def _scrape_detail_page(self, job_url: str) -> dict[str, str]:
        ...
        try:
            ...
            # Give the SPA time to render the detail overlay
    ...
    def _make_honeywell_job_url(self, source_url: str, href: str) -> str:
    ...
    def _clean_location_text(self, text: str) -> str:
        ...
        noise_values = {"locations", "location", "and more"}
    ...
    async def _fallback_links(
        ...
        for a in anchors[:max_jobs]:
            ...
            url = self._make_honeywell_job_url(source_url, href)
