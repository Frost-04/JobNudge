...
class AnalogDevicesScraper(BaseScraper):
    """
    Scraper for Analog Devices job search pages (Workday platform).
        ...
        section[data-automation-id="jobResults"]
          ul[aria-label^="Page"][role="list"]
            li
              ...
              [data-automation-id="subtitle"] li   (R number)
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if not cards:
                self.logger.warning("No Analog Devices job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Analog Devices job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list ("R263019").
        job_id = ""
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Analog Devices"),
            title=title,
            location=location,
            url=url,
