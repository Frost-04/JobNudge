...
class QuinceScraper(BaseScraper):
    """
    Scraper for Quince job board (powered by Greenhouse).

    Greenhouse powers career pages for many companies. The listing page is
    server-rendered HTML with department-grouped tables. See
    techniques_for_scraping.md §2.11 for greenhouse-specific patterns.
    ...
    Technique — Server-rendered table scraping:
    Greenhouse pages are pure server-rendered HTML.  No JS rendering needed
    for the cards — ``wait_until="domcontentloaded"`` is sufficient.  Cards
    are HTML table rows, which makes extraction straightforward with BS4.
    """
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Quince job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Quince"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        ...
        https://job-boards.greenhouse.io/quince/jobs/4586301008
    ...
    def _extract_job_id_from_url(self, url: str) -> str:
        """
        Greenhouse URLs: https://job-boards.greenhouse.io/quince/jobs/4586301008
        """
        if not url:
            return ""
