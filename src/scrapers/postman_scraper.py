...
class PostmanScraper(BaseScraper):
    """
    Scraper for Postman job board (powered by Greenhouse).

    Greenhouse standard layout — server-rendered HTML with department-grouped
    tables. Same platform as Anthropic, Glean, InMobi, Quince.

    The listing URL uses query-string office/department filters to show only
    relevant roles in India (Hyderabad, Bengaluru).
    ...
    Expected detail page structure (separate page on click):
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Postman job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Postman"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_job_id_from_url(self, url: str) -> str:
        """
        Greenhouse URLs: https://job-boards.greenhouse.io/postman/jobs/7687341003
        """
        if not url:
            return ""
    ...
    async def _fallback_links(self, page: Page, source_url: str, max_jobs: int) -> list[Job]:
        ...
        for link in soup.select('a[href*="/postman/jobs/"]'):
            if len(jobs) >= max_jobs:
                break

            href = link.get("href")

            if not href or "/postman/jobs/" not in str(href):
                continue
    ...
    def _clean_multiline_text(text: str) -> str:
        ...
        for line in text.splitlines():
            clean_line = PostmanScraper._clean_text(line)
