...
class WalmartScraper(BaseScraper):
    """
    Scraper for Walmart Global Tech careers (Workday platform, wd504).

    The search results page renders job cards in a right-side panel layout.
    Clicking a card loads the description in an adjacent panel — we scrape
    detail pages directly instead.
        ...
        section[data-automation-id="jobResults"]
          ul[aria-label^="Page"][role="list"]
            li
              ...
              [data-automation-id="subtitle"] li   (R-XXXXXXX)
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            # Extra settle time for SPA render (cards may load post-networkidle).
            ...
            if not cards:
                self.logger.warning("No Walmart job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Walmart job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list ("R-2450394").
        job_id = ""
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Walmart"),
            title=title,
            location=location,
            url=url,
