...
class IntelScraper(BaseScraper):
    """
    Scraper for Intel Careers job search pages (Workday platform).
    ...
    async def scrape(self) -> list[Job]:
        ...
        try:
            ...
            if not cards:
                self.logger.warning("No Intel job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                try:
                    ...
                    self.logger.warning(
                        "Failed to enrich Intel job detail %s: %s",
                        job.url,
                        exc,
                    )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        # Job ID from the subtitle list (JR number like "JR0284344").
        job_id = ""
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Intel"),
            title=title,
            location=location,
            url=url,
