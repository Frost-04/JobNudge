...
class InfineonScraper(BaseScraper):
    """
    Scraper for Infineon Careers search result pages (Phenom People platform).
    ...
    div[data-test-id="job-listing"]
      a[href^="/careers/job/"]
        ...
        div.fieldValue-3kEar  (HRC req ID — second fieldsContainer, no icon)
        div.subData-13Lm1     (posted date — often empty)
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Infineon job detail page %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Infineon"),
            title=title,
            location=location,
            url=link,
    ...
    def _extract_link(self, card: Tag, source_url: str) -> str:
        ...
        return self._make_infineon_job_url(source_url, str(href))
    ...
    def _extract_location(self, card: Tag) -> str:
        """
        Infineon cards have fieldValue elements in order:
        1. Location (inside fieldsContainer with an SVG icon)
        2. HRC req ID (second fieldsContainer, no icon)
        3. (optional) Work type like "Hybrid"
    ...
    def _extract_job_id(self, card: Tag, link: str) -> str:
        """Infineon listing href looks like: /careers/job/563808970647124"""

        if link:
            job_id = self._extract_infineon_job_id_from_url(link)
    ...
    def _make_infineon_job_url(self, source_url: str, href: str) -> str:
    ...
    def _extract_infineon_job_id_from_url(self, url: str) -> str:
        if not url:
            return ""
    ...
    async def _fallback_links(
        ...
        for anchor in anchors[:max_jobs]:
            ...
            job_url = self._make_infineon_job_url(source_url, str(href))
            job_id = self._extract_infineon_job_id_from_url(job_url)
