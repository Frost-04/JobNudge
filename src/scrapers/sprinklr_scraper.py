...
class SprinklrScraper(BaseScraper):
    """
    Scraper for Sprinklr Careers job search pages (Workday platform).
        ...
        section[data-automation-id="jobResults"]
          ul[aria-label="Page ..."][role="list"]
            li
              ...
              [data-automation-id="subtitle"] li   (job ID like "113071-JOB")
        ...
        try:
            ...
            try:
                ...
                if await accept_btn.is_visible(timeout=5000):
                    ...
                    self.logger.debug("Accepted cookies on Sprinklr page.")
                    await page.wait_for_timeout(5000)
            except Exception:
                self.logger.debug("No cookie consent popup on Sprinklr page.")
            ...
            if not cards:
                self.logger.warning("No Sprinklr job cards found.")
                return jobs
            ...
            for card in cards[:max_jobs]:
                ...
                if self._should_exclude(job.title):
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "Failed to enrich Sprinklr job detail %s: %s",
                            job.url,
                            exc,
                        )
    ...
    def _parse_card(self, card: Tag, source_url: str) -> Job | None:
        ...
        return Job(
            job_id=job_id,
            company=self.company_config.get("name", "Sprinklr"),
            title=title,
            location=location,
            url=url,
